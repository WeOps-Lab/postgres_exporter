...
kind: StatefulSet
metadata:
  name: pg-exporter-cluster-ha-{{ARCHITECTURE}}-{{VERSION}}
  namespace: postgres
spec:
  serviceName: pg-exporter-cluster-ha-{{ARCHITECTURE}}-{{VERSION}}
  replicas: 1
  selector:
    matchLabels:
      app: pg-exporter-cluster-ha-{{ARCHITECTURE}}-{{VERSION}}
  template:
    metadata:
      annotations:
    ...
    spec:
      ...
      - name: pg-exporter-cluster-ha-{{ARCHITECTURE}}-{{VERSION}}
        ...
        - name: DATA_SOURCE_NAME
          value: "postgresql://postgres:Weops123!@pg-cluster-ha-{{VERSION}}-postgresql-{{ARCHITECTURE}}.postgres:5432?sslmode=disable"
        resources:
          requests:
            cpu: 100m
            memory: 100Mi
